import {
  TCreateType,
  useAssistanceOfficialLetter,
} from "@/hooks/analizer/assistant/useAssistanceOfficialLetter";
...
const OfficailLetter = () => {
  const [offical, setOffical] = useState("");
  const letter = useAssistanceOfficialLetter();

  const [form] = useForm();

  const handleEntity = async (values: TCreateType) => {
    const res = await letter.mutateAsync(values);
  ...
  const direction = getDirection(offical || "");
  return (
    <>
