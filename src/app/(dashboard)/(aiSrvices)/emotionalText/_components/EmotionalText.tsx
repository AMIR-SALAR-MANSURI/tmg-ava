import {
  TCreateType,
  useAssistanceEmotionaText,
} from "@/hooks/analizer/assistant/useAssistanceEmotionaText";
...
const EmotionalText = () => {
  ...
  const emotion = useAssistanceEmotionaText();

  const [form] = useForm();

  const handleUnthical = async (values: TCreateType) => {
    const res = await emotion.mutateAsync(values);
    if (res.isSuccess == true) {
      setEmotional(res.data);
    }
  };
  ...
  const direction = getDirection(emotional || "");
  return (
    <>
