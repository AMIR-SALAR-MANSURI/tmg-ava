import {
  TCreateType,
  useAssistanceEntityName,
} from "@/hooks/analizer/assistant/useAssistanceEntityName";
...
const EntityName = () => {
  ...
  const entity = useAssistanceEntityName();

  const [form] = useForm();

  const handleEntity = async (values: TCreateType) => {
    const res = await entity.mutateAsync(values);
  ...
  const direction = getDirection(entites || "");
  return (
    <>
